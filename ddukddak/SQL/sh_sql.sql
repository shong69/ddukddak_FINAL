...
NATURAL JOIN ORDER_DETAIL od 
NATURAL JOIN "OPTION" o
ORDER BY CART_ID, ORDER_ITEM_NO;
...
SELECT 
	...
	SELECT COUNT(*)
		...
		ORDER BY p.PRODUCT_NO DESC;
		    ...
		    o.ORDER_NO,
		    uf.UPLOAD_IMG_PATH || uf.UPLOAD_IMG_RENAME AS THUMBNAIL,
		    LISTAGG(o2.OPTION_NAME || ': ' || o2.OPTION_VALUE, ' / ') 
		    WITHIN GROUP (ORDER BY o.ORDER_NO) AS ORDER
		FROM PRODUCT p
		JOIN UPLOAD_FILE uf ON p.PRODUCT_NO = uf.PRODUCT_NO
		JOIN ORDER_DETAIL od ON p.PRODUCT_NO = od.PRODUCT_NO
		JOIN ORDERS o ON od.ORDER_NO = o.ORDER_NO
    ...
    uf.UPLOAD_IMG_PATH || uf.UPLOAD_IMG_RENAME AS THUMBNAIL,
    LISTAGG(o2.OPTION_NAME || ': ' || o2.OPTION_VALUE, ' / ') 
        WITHIN GROUP (ORDER BY o.ORDER_NO) AS ORDER_OPTION
FROM PRODUCT p
JOIN UPLOAD_FILE uf ON p.PRODUCT_NO = uf.PRODUCT_NO
JOIN ORDER_DETAIL od ON p.PRODUCT_NO = od.PRODUCT_NO
JOIN ORDERS o ON od.ORDER_NO = o.ORDER_NO
...
UPDATE ORDER_DETAIL od
	    SET od.ORDER_STATUS = '출고대기'
	    WHERE od.ORDER_NO = 250
	    AND od.PRODUCT_NO = 21
		AND od.ORDER_ITEM_NO = 58
	    AND EXISTS (
	        SELECT 1
	        FROM PRODUCT p
	        JOIN UPLOAD_FILE uf ON p.PRODUCT_NO = uf.PRODUCT_NO
	        ...
	        JOIN ORDERDETAIL_OPTION oo ON od.ORDER_ITEM_NO = oo.ORDER_ITEM_NO
	        JOIN "OPTION" o2 ON oo.OPTION_NO = o2.OPTION_NO
	        WHERE p.PRODUCT_NO = od.PRODUCT_NO
	          AND uf.UPLOAD_IMG_ORDER = 0
	          AND p.PARTNER_NO = 1
	          AND p.PRODUCT_NAME = '로우 저상형 침대 프레임 깔판 (매트선택) 화이트/오크 SS/Q(K호환)'
	    );
...
COMMIT;
	...
	UPDATE ORDER_DETAIL od
		    SET od.ORDER_STATUS = '주문취소'
		    WHERE od.ORDER_NO = 245
		    AND od.ORDER_STATUS = '출고대기'
			AND od.ORDER_ITEM_NO = 46
		    AND EXISTS (
		        SELECT 1
		        FROM PRODUCT p
		        JOIN UPLOAD_FILE uf ON p.PRODUCT_NO = uf.PRODUCT_NO
		        ...
		        AND uf.UPLOAD_IMG_ORDER = 0
		        AND p.PARTNER_NO = 1
		        AND p.PRODUCT_NAME = '쿠폰가 199,900원+세트할인 / 비앙카 LED 수납침대+매트 선택(SS/Q/K)'
		    );
...
NATURAL JOIN "MEMBER";
	...
	NATURAL JOIN UPLOAD_FILE uf
WHERE BOARD_CODE = 2;

COMMIT;

SELECT * FROM UPLOAD_FILE uf WHERE CATEGORY = 6;
...
INSERT INTO UPLOAD_FILE uf VALUES (SEQ_UPLOAD_IMG_NO.NEXTVAL, '/images/main/', 'noImageProject.png', 'noImageProject.png', 0, 6, NULL, NULL, NULL, NULL);
	
	INSERT INTO PROJECT p VALUES (SEQ_PROJECT_NO.NEXTVAL, 'test1', 12, DEFAULT);

UPDATE UPLOAD_FILE SET UPLOAD_IMG_PATH = '/images/tip/' WHERE UPLOAD_IMG_NO IN (581, 583, 622, 624, 626, 689, 584, 625, 582, 585, 623, 627);


SELECT BOARD_NO, BOARD_TITLE, MEMBER_NICKNAME, READ_COUNT,
		(SELECT UPLOAD_IMG_PATH || UPLOAD_IMG_RENAME FROM "UPLOAD_FILE" U WHERE BOARD_NO = U.BOARD_NO AND UPLOAD_IMG_ORDER = 0) THUMBNAIL
		FROM "COMMUNITY"
		JOIN "MEMBER" USING (MEMBER_NO)
 ...
 SELECT 
		...
		JOIN CART c ON p.PRODUCT_NO = c.PRODUCT_NO
		JOIN "MEMBER" m ON c.MEMBER_NO = m.MEMBER_NO 
		WHERE od.ORDER_STATUS IN ('주문거절', '구매확정', '출고거절', '주문취소')
		AND uf.UPLOAD_IMG_ORDER = 0
		AND p.PARTNER_NO = 1
		GROUP BY 
		    p.PRODUCT_NO, 
