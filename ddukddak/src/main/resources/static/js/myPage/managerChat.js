...
// 제품 정보
function choose1(event){
    createChatMessage("제품 정보는\n해당 제품의 상세 페이지에서 열람 가능합니다.\n자세한 문의는 제품 페이지 내의 문의 내역에 질문을 남겨주세요.");
    completeSelection(event);
    customerChatting(); // customerChatting 함수 다시 호출
}

// 주문, 결제 정보
function choose2(event){
    createChatMessage("원하시는 주문 번호를 입력해주세요.");
    inputChatting.removeAttribute('disabled');
    ...
    document.querySelector("#send").addEventListener("click", async () => {
        ...
        createChatMessage(orderText);
        completeSelection(event);
    });

    customerChatting(); // customerChatting 함수 호출
}


// 적립금 및 포인트
function choose3(event){
    createChatMessage(`회원님께서 구매하신 총 금액의 1%가 포인트로 쌓이게 됩니다.\n회원님이 현재 보유하신 포인트 금액은 ${memberPoint}pt 입니다`);
    completeSelection(event);
    customerChatting(); // customerChatting 함수 다시 호출
}
...
// 게시글 작성 및 관리
function choose4(event){
    createChatMessage("뚝딱뚝딱에서는\n집들이 게시판과 노하우 게시판에 게시글을 작성할 수 있습니다.\n집들이 게시판에서는 회원님의 집 내부와 인테리어를 뽐내주세요!\n마찬가지로 노하우 게시판에서도 회원님이 가진 특별한 집 꾸미기 지식을 뽐내주시면 됩니다~");
    completeSelection(event);
    customerChatting(); // customerChatting 함수 다시 호출
}
...
// 전문가 상담
function choose5(event){
    createChatMessage("포트폴리오 게시판에서는 뚝딱뚝딱과 함께하고 있는 시공사, 인테리어 회사들을 둘러 볼 수 있습니다.\n포트폴리오를 살펴보시고 전문가에게 채팅해주세요! 빠르게 답변해드릴게요 :)");
    completeSelection(event);
    customerChatting(); // customerChatting 함수 다시 호출
}
...
// 3d 홈디자인 기능
function choose6(event){
    createChatMessage("뚝딱뚝딱에서는 3d 기능을 사용하여\n내 방 꾸미기를 미리 해 볼 수 있습니다!\n원하는 요소를 배치하여 마음대로 꾸며보세요!\n결과물을 다운받아 시공사에게 컨택할 수 있습니다");
    completeSelection(event);
    customerChatting(); // customerChatting 함수 다시 호출
}
...
// 사이트 사용 문제
function choose7(event){
    createChatMessage("뚝딱뚝딱 사이트를 이용하며 불편하신 점은\n아래 메일 주소로 연락 부탁드립니다.\n대표 이메일 : saem.hong.95@gmail.com");
    completeSelection(event);
    customerChatting(); // customerChatting 함수 다시 호출
}
...
// 문의 종료
function choose0(event){
    createChatMessage("문의 종료를 원하신다면 상단 우측의 '문의 종료' 버튼을 클릭해주세요");
    completeSelection(event);
    customerChatting(); // customerChatting 함수 다시 호출
}
